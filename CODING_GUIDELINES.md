...
5. There should be only 1 command necessary to run all unit tests (usually `make test` or `./test`); keep integration tests, performance tests and fuzzing tests as separate suites, `./test_all` or `make test_all` should additionally run those as well.

...
testScoringDisplay = do
  ...
  let criteria1 = MkCriteriaScore 8.0 7.5 6.0 8.5
  let criteria2 = MkCriteriaScore 7.0 8.0 7.5 6.5
  let score1 = MkScore participant1 criteria1 (Application.Judging.calculateTotalScore criteria1)
  let score2 = MkScore participant2 criteria2 (Application.Judging.calculateTotalScore criteria2)
...
testWinnerDeclaration = do
  ...
  let highCriteria = MkCriteriaScore 9.0 8.5 8.0 9.0  -- 34.5/40 total
  let lowCriteria = MkCriteriaScore 6.0 5.5 6.5 7.0   -- 25.0/40 total
  let winnerScore = MkScore participant1 highCriteria (Application.Judging.calculateTotalScore highCriteria)
  let loserScore = MkScore participant2 lowCriteria (Application.Judging.calculateTotalScore lowCriteria)
  let scores = [winnerScore, loserScore]

  putStrLn "Testing winner calculation..."
  if winnerScore.totalScore > loserScore.totalScore
    then do
      putStrLn ("✅ Winner correctly identified: " ++ show participant1.model)
      putStrLn ("Winner score: " ++ show winnerScore.totalScore)
      putStrLn ("Loser score: " ++ show loserScore.totalScore)
    else putStrLn "❌ Winner calculation failed"

  -- Test dramatic declaration logic (mocked for testing)
  putStrLn "Testing winner declaration logic..."
  let mockDeclaration = "🚨 BEHOLD! ChatGPT4o DOMINATES the battlefield with DEVASTATING arguments! 🏆"
  putStrLn "Sample dynamic declaration (mocked):"
  putStrLn mockDeclaration

  -- Verify declaration contains expected framework elements
  if isInfixOf "ChatGPT4o" mockDeclaration &&
     isInfixOf "DOMINATES" mockDeclaration
...
testScoreFormatting = do
  ...
  putStrLn ("Original: 31.319999999999997 → Formatted: " ++ Application.Judging.formatScore testScore1)
  putStrLn ("Original: 29.650000000000002 → Formatted: " ++ Application.Judging.formatScore testScore2)
  putStrLn ("Original: 25.0 → Formatted: " ++ Application.Judging.formatScore testScore3)
  putStrLn ("Original: 18.95 → Formatted: " ++ Application.Judging.formatScore testScore4)
